...
mk_fat() {
	...
	'[ -e "${archive_name%%.*}" ] && { echo "the file \"${archive_name%%.*}\" already exists"; exit 2; };' \
       	'tail +3 $0|tar kxv'"$format_tar_flag"' --; exit'|cat - "${TMP_DIR}/${tar_name}.${ext}" > "$WORKDIR/${tar_name}.fat"
	chmod +x "$WORKDIR/${tar_name}.fat"
}
...
mk_archive "$name" $ext
mk_fat $format_tar_flag "$name" $ext
